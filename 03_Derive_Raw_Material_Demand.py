...
demand_df = spark.read.table(f"marco_data.{dbName}.part_level_demand_with_forecasts")
sku_mapper = spark.read.table(f"marco_data.{dbName}.sku_mapper")
bom = spark.read.table(f"marco_data.{dbName}.bom")
...
display(spark.sql(f"select distinct SKU from marco_data.{dbName}.part_level_demand_with_forecasts"))
...
spark.sql(f"DROP TABLE IF EXISTS marco_data.{dbName}.forecast_raw")
...
# Align the schemas
demand_raw_df = demand_raw_df.withColumn("Demand_SKU", demand_raw_df["Demand_SKU"].cast("string"))

# Write the data with schema merging enabled
demand_raw_df.write \
    .mode("overwrite") \
    .format("delta") \
...
spark.sql(f"DROP TABLE IF EXISTS marco_data.{dbName}.forecast_raw")
spark.sql(f"CREATE TABLE marco_data.{dbName}.forecast_raw USING DELTA LOCATION '{forecast_df_delta_path}'")

# COMMAND ----------

display(spark.sql(f"SELECT * FROM marco_data.{dbName}.forecast_raw"))
